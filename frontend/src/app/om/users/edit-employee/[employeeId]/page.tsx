...
export default function EditEmployeeRoute() {
  ...
  if (loading)
    return (
      <section aria-busy="true" className="animate-pulse p-5 sm:p-7 lg:p-8">
        <div className="h-8 w-52 rounded bg-slate-200" />
        <div className="mt-3 h-5 w-80 max-w-full rounded bg-slate-100" />
        <div className="mt-7 min-h-135 rounded-2xl border border-slate-200 bg-white p-6">
          <div className="h-10 w-full rounded bg-slate-100" />
          <div className="mt-7 space-y-4">
            {[0, 1, 2, 3, 4].map((item) => (
              <div key={item} className="h-11 rounded bg-slate-100" />
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="mt-5">
          {activeTab === "Basic" ? (
            <BasicEmployeeForm
              ...
              mode="edit"
              employeeId={userId}
              initialData={employee as EmployeeFormValues}
              formId={formId}
              onUpdated={() => {
          ...
          <div className={activeTab === "Payroll" ? "" : "hidden"}>
            <PayrollForm
              ...
              onNext={() => setActiveTab("Basic")}
              onExit={() => router.push("/om/users")}
              onSaved={() => {
                setSuccess("Payroll records updated successfully.");
                refreshEmployee();
              }}
            />
          </div>
