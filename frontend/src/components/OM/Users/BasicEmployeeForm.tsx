...
import { uploadImage as uploadCloudImage } from "@/Services/upload";
...
export function BasicEmployeeForm({
  ...
  const setPrimaryRole = (_: string, role: string) =>
    setValues((current) => ({ ...current, role, secondaryRole: "" }));
  const selectedPrimaryRole =
    values.role as keyof typeof secondaryRolesByPrimaryRole;
  const secondaryRoleOptions =
    secondaryRolesByPrimaryRole[selectedPrimaryRole] ?? [];
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (
      values.password &&
      (values.password as string) !== (values.confirmPassword as string)
    ) {
    ...
    try {
      ...
      if (mode === "edit") {
        ...
        if (!updateTarget)
          throw new Error("Employee ID is missing from this edit request.");
