...
const includeSites = { sites: { include: { site: true } } };
...
usersRoutes.post("/", async (c) => {
    ...
    if (!input.fullName?.trim() || !input.employeeId?.trim() || !input.password || !input.nationality?.trim() || !input.phone?.trim() || !input.role?.trim())
    ...
    if (input.deploymentSiteId && !await prisma.site.findUnique({ where: { id: input.deploymentSiteId }, select: { id: true } }))
        return c.json({ message: "Selected deployment site does not exist." }, 400);
    if (siteIds.length && await prisma.site.count({ where: { id: { in: siteIds } } }) !== siteIds.length)
