...
export function login(payload: { employeeId: string; company: string; password: string }) {
...
export function getSession() { return request<{ user: SignedInUser; dashboardPath: string }>("/api/auth/session"); }
export function logout() { return request<{ message: string }>("/api/auth/logout", { method: "POST" }); }
