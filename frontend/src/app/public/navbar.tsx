    ...
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-slate-50/90 backdrop-blur-xl">
      ...
      {isOpen && (
        <div
          ...
          className="border-t border-slate-200 bg-slate-50 px-5 py-5 lg:hidden"
        >
          <div className="mx-auto flex max-w-7xl flex-col gap-1">
            {navigation.map(([label, href]) => (
              <a
                ...
                href={href}
                onClick={closeMenu}
                className="rounded-lg px-3 py-3 text-sm font-medium text-slate-700 hover:bg-slate-200"
              >
                {label}
              </a>
            ))}
            <Link
              href="/login"
              onClick={closeMenu}
              className="rounded-lg px-3 py-3 text-sm font-semibold text-slate-900 hover:bg-slate-200"
            >
              Login
            </Link>
            <a
