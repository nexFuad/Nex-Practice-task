...
type ActionInput = { siteId?: string; shiftId?: string; latitude?: number; longitude?: number; accuracy?: number; photoUrl?: string };
...
const timing = (time: Date, shiftTime: string) => {
...
async function details(sessionUserId: string, input: ActionInput) {
  if (!input.siteId || !input.shiftId || !input.photoUrl?.trim() || !numeric(input.latitude) || !numeric(input.longitude) || !numeric(input.accuracy)) throw new Error("Site, shift, live photo and current location are required.");
  const user = await prisma.user.findUnique({ where: { id: sessionUserId }, include: { basic: true, sites: { include: { site: true } } } });
  if (!user) throw new Error("Officer account was not found.");
  const site = await prisma.site.findUnique({ where: { id: input.siteId } });
  const shift = await prisma.shift.findUnique({ where: { id: input.shiftId } });
  if (!site || site.status !== "ACTIVE") throw new Error("Selected site is unavailable.");
  if (!shift || shift.status !== "ACTIVE") throw new Error("Selected shift is unavailable.");
  if (user.sites.length && !user.sites.some(({ siteId }) => siteId === site.id)) throw new Error("This site is not assigned to you.");
  if (shift.siteId && shift.siteId !== site.id) throw new Error("This shift is not available for the selected site.");
  if (site.latitude === null || site.longitude === null) throw new Error("This site has no configured check-in location. Contact your operations manager.");
  const distance = distanceMeters(input.latitude!, input.longitude!, site.latitude, site.longitude);
  if (distance > site.geofenceRadius) throw new Error(`You are outside the allowed check-in area for this site. Distance from site: ${Math.round(distance)} m. Allowed radius: ${site.geofenceRadius} m.`);
...
officerAttendanceRoutes.get("/options", async (c) => {
  ...
  if (!user) return c.json({ message: "Officer not found." }, 404);
...
officerAttendanceRoutes.get("/active", async (c) => {
  const session = getSession(c)!; const { start, end } = dayRange();
...
officerAttendanceRoutes.get("/history", async (c) => {
  const session = getSession(c)!; const page = Math.max(1, Number(c.req.query("page")) || 1); const query = c.req.query("query")?.trim(); const type = c.req.query("type"); const date = c.req.query("date");
  const where = { userId: session.userId, ...(date ? { shiftDate: { gte: new Date(`${date}T00:00:00.000Z`), lt: new Date(`${date}T23:59:59.999Z`) } } : {}), ...(type === "CHECK_IN" ? { checkOutAt: null } : type === "CHECK_OUT" ? { checkOutAt: { not: null } } : {}), ...(query ? { OR: [{ siteName: { contains: query, mode: "insensitive" as const } }, { siteCode: { contains: query, mode: "insensitive" as const } }, { shiftType: { contains: query, mode: "insensitive" as const } }] } : {}) };
...
officerAttendanceRoutes.post("/check-in", async (c) => {
  try { const session = getSession(c)!; const input = await c.req.json<ActionInput>(); const { user, site, shift, distance } = await details(session.userId, input); const { start, end } = dayRange();
    const active = await prisma.attendanceRecord.findFirst({ where: { userId: user.id, checkInAt: { not: null }, checkOutAt: null } });
    if (active) return c.json({ message: "Check out from your active duty before starting another check-in." }, 409);
    const checkedInAt = new Date(); const checkInTiming = timing(checkedInAt, shift.startTime);
  ...
  } catch (error) { return c.json({ message: error instanceof Error ? error.message : "Unable to check in." }, 400); }
});
officerAttendanceRoutes.post("/check-out", async (c) => {
  try { const session = getSession(c)!; const input = await c.req.json<ActionInput>(); const { site, shift, distance } = await details(session.userId, input); const { start, end } = dayRange();
    ...
    if (!record) return c.json({ message: "A valid active check-in for this site and shift is required before check-out." }, 409);
    const checkedOutAt = new Date(); const checkOutTiming = timing(checkedOutAt, shift.endTime);
  ...
  } catch (error) { return c.json({ message: error instanceof Error ? error.message : "Unable to check out." }, 400); }
});
