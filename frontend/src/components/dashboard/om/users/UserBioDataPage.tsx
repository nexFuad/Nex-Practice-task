...
export function UserBioDataPage() {
  ...
  const { data: employee = null, isLoading, error: queryError } = useQuery<EditableEmployee>({
  ...
  if (!employee) return <p className="p-8 text-sm text-slate-500">Officer particulars are unavailable.</p>;
...
function OfficerParticularsSkeleton() {
  return <section aria-busy="true" className="min-h-full animate-pulse bg-slate-50 p-4 sm:p-7 lg:p-8"><div className="mx-auto min-h-[297mm] w-full max-w-[210mm] bg-white p-[7mm] shadow-sm sm:p-[16mm]"><div className="flex items-center justify-between border-b-2 border-slate-200 pb-8"><div className="size-16 rounded-xl bg-slate-200" /><div className="space-y-3"><div className="ml-auto h-6 w-48 rounded bg-slate-200" /><div className="h-4 w-72 rounded bg-slate-100" /></div></div><div className="mx-auto mt-10 h-9 w-64 rounded bg-slate-200" /><div className="mt-12 h-7 w-52 rounded bg-slate-200" /><div className="mt-7 grid gap-6 sm:grid-cols-2"><div className="space-y-4">{[0, 1, 2, 3, 4, 5].map((item) => <div key={item} className="h-4 rounded bg-slate-100" />)}</div><div className="space-y-4">{[0, 1, 2, 3, 4, 5].map((item) => <div key={item} className="h-4 rounded bg-slate-100" />)}</div></div></div></section>;
}
