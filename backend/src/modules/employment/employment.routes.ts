...
const recordData = (input: RecordInput) => ({ dateLeft: date(input.dateLeft), probationPeriod: text(input.probationPeriod), noticePeriod: text(input.noticePeriod), status: text(input.status) ?? "Active", notificationDate: date(input.notificationDate), confirmationDate: date(input.confirmationDate), remarks: text(input.remarks) });
const findUser = (employeeId?: string) => employeeId ? prisma.user.findUnique({ where: { employeeId: employeeId.trim() }, select: { id: true } }) : null;

const listEmployment = async (c: Context) => {
  const employeeId = c.req.query("employeeId");
  const user = await findUser(employeeId);
  if (employeeId && !user) return c.json({ message: "Employee not found." }, 404);
...
employmentRoutes.post("/commit", async (c) => {
  ...
  const user = await findUser(employeeId);
  if (!user) return c.json({ message: "Save Basic information before saving employment records." }, 400);
...
employmentRoutes.post("/records", async (c) => {
  ...
  if (!user) return c.json({ message: "Employee not found." }, 404);
