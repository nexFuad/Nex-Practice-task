...
export function UserBioDataPage() {
  ...
  return <section className="bio-data-page min-h-full bg-slate-50 p-5 sm:p-7 lg:p-8"><style jsx global>{`@page { size: A4 portrait; margin: 0; } @media print { html, body { background: #fff !important; } body * { visibility: hidden !important; } .bio-report, .bio-report * { visibility: visible !important; } .bio-report { position: absolute !important; inset: 0 !important; width: 210mm !important; min-height: 297mm !important; margin: 0 !important; padding: 16mm !important; box-shadow: none !important; } }`}</style><header className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden"><button type="button" onClick={() => router.push("/om/users")} className="inline-flex h-10 items-center gap-2 rounded-md px-2 text-sm font-medium hover:bg-slate-100"><ArrowLeft className="size-5" />Back to users</button><div className="flex flex-wrap gap-3"><button type="button" onClick={printReport} className="inline-flex h-10 items-center gap-2 rounded-md border border-slate-200 bg-white px-4 text-sm font-medium shadow-sm hover:bg-slate-50"><Printer className="size-5" />Print / Save PDF</button><button type="button" onClick={() => downloadOfficerParticularsPdf({ employeeName: display(employee.fullName), companyName: company.name, sections: pdfSections })} className="inline-flex h-10 items-center gap-2 rounded-md bg-neutral-900 px-4 text-sm font-medium text-white shadow-sm hover:bg-neutral-800"><Download className="size-5" />Download PDF</button></div></header><article className="bio-report mx-auto min-h-[297mm] w-full max-w-[210mm] bg-white p-[12mm] text-black shadow-sm sm:p-[16mm]"><header className="grid grid-cols-[1fr_auto] gap-6 border-b-2 border-black pb-8"><div className="flex items-center gap-3"><span className="grid size-16 place-items-center rounded-xl bg-blue-600 text-white"><ShieldCheck className="size-9" /></span><span className="text-xl font-bold text-slate-800">{company.name}</span></div><div className="max-w-[78mm] text-sm leading-6"><h1 className="text-xl font-bold">{company.name}</h1><p>{company.address}</p><div className="flex flex-wrap gap-x-8"><p>Tel: {company.telephone}</p><p>Fax: {company.fax}</p></div></div></header><h2 className="my-10 text-center text-3xl font-bold">Officer Particulars</h2><section><h3 className="mb-6 text-2xl font-bold">Personal Details</h3><div className="grid grid-cols-[minmax(0,1fr)_34mm] gap-7"><div className="grid gap-x-10 gap-y-6 md:grid-cols-2"><DetailList rows={personalLeft} /><DetailList rows={personalRight} /></div><Photo imageUrl={employee.profileImageUrl} name={display(employee.fullName)} /></div><div className="mt-8 max-w-[138mm] space-y-3 text-sm"><Field label="Address" value={display(employee.address)} /><Field label="Education Details" value="-" /></div></section><section className="mt-12"><h3 className="text-lg font-bold">Employment Details</h3>{employment.length ? <ol className="mt-4 list-none space-y-2 text-sm">{employment.map((entry, index) => <li key={`${entry.dateJoin}-${index}`} className="break-words">{index + 1}) <span className="font-medium">{display(entry.status)}</span> — {formatDate(entry.dateJoin)}{entry.dateLeft ? ` to ${formatDate(entry.dateLeft)}` : ""}{entry.remarks ? ` — ${entry.remarks}` : ""}</li>)}</ol> : <p className="mt-4 text-sm">-</p>}</section><section className="mt-8"><h3 className="text-lg font-bold">Security Courses Attended:</h3>{courses.length ? <ol className="mt-4 list-decimal space-y-1 pl-5 text-sm">{courses.map((course) => <li key={course}>{course}</li>)}</ol> : <p className="mt-4 text-sm">-</p>}</section><footer className="mt-20 break-inside-avoid text-xs italic leading-5 text-slate-600"><p className="font-bold not-italic text-orange-500">DISCLAIMER</p><p className="mt-1">To safeguard your personal data from unauthorised access, collection, use, disclosure, copying, modification, disposal or similar risks, we have introduced appropriate administrative, physical and technical measures such as up-to-date antivirus protection, encryption and the use of privacy filters.</p><p className="mt-4">You should be aware, however, that no method of transmission over the Internet or method of electronic storage is completely secure. While security cannot be guaranteed, we strive to protect the security of your information and are constantly reviewing and enhancing our information security measures.</p><div className="mt-5 flex flex-wrap justify-between gap-x-8 gap-y-2 not-italic text-sm text-black"><span>Website: {company.website}</span><span>Contact Email: {company.email}</span></div></footer></article></section>;
}
