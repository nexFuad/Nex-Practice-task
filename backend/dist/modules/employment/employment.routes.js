...
const recordData = (input) => ({ employeeId: text(input.employeeId), dateLeft: date(input.dateLeft), probationPeriod: text(input.probationPeriod), noticePeriod: text(input.noticePeriod), status: text(input.status) ?? "Active", notificationDate: date(input.notificationDate), confirmationDate: date(input.confirmationDate), remarks: text(input.remarks) });
