...
import { getSiteOptions } from "@/Services/site";
...
export function UserActionDialog({
  ...
  const employeeQuery = useQuery<EditableEmployee>({
    queryKey: ["users", user.databaseId],
    queryFn: () => getUser(user.databaseId),
  });
  const scheduleQuery = useQuery<UserScheduleRecord[]>({
    queryKey: ["users", user.databaseId, "schedule"],
    queryFn: () => getUserSchedule(user.databaseId),
    enabled: action === "View Schedule",
  });
  const sitesQuery = useQuery({
    queryKey: ["site-options", "ACTIVE"],
    queryFn: () => getSiteOptions("ACTIVE"),
    enabled: action === "Assign to Sites",
  });
  const employee = employeeQuery.data ?? null;
  const schedule = scheduleQuery.data ?? [];
  const sites = (sitesQuery.data ?? []).map(({ id, name, code, address }) => ({
    id,
    name,
    code,
    address,
  }));
  const effectiveSelectedSites =
    selectedSites ?? (employee?.siteIds as string[] | undefined) ?? [];
  ...
  const employmentRecords =
    (employee?.employmentRecords as
      | { id: string; dateJoin?: string }[]
      | undefined) ?? [];
  const employmentRecord = employmentRecords.find((record) =>
    Boolean(record.dateJoin),
  );
  ...
  const submit = async () => {
    ...
    try {
      if (action === "Assign to Sites") {
        ...
        await setUserStatus(user.databaseId, "SUSPENDED");
        onChanged();
        ...
        if (!employmentRecord)
          throw new Error(
            "No employment record found. Add a Date Join before marking this user as resigned.",
          );
        if (!lastWorkingDay)
          throw new Error("Please select the last working date.");
  ...
  return (
    ...
    >
      ...
      >
        <header
          className={`flex items-center justify-between ${action === "View Assigned Sites" ? "shrink-0" : "border-b border-slate-100 pb-3"}`}
        >
          <div>
            <h2 className="flex items-center gap-2 text-lg font-semibold text-slate-900">
              {action === "Assign to Sites" && <Building2 className="size-5" />}
            ...
            {action === "View Assigned Sites" && (
              <p className="mt-2 text-sm text-slate-500">
                View and manage site assignments for {user.name}
              </p>
            )}
            {action === "Assign to Sites" && (
              <p className="mt-2 text-sm text-slate-500">
                Select which sites this user can access and work at.
              </p>
            )}
          </div>
          <button
        ...
        <div
          className={`${action === "View Assigned Sites" ? "flex-1 overflow-y-auto pt-5 text-sm text-slate-700" : "max-h-[60vh] overflow-y-auto py-4 text-sm text-slate-700"}`}
        >
          {action === "View Assigned Sites" && (
            <SiteAssignments
              user={user}
          ...
          {action === "Resign User" && (
            <div className="space-y-4">
              <p>
                Are you sure you want to mark <b>{user.name}</b> as resigned?
                Please specify their last working date.
              </p>
              {employmentRecord ? (
                <label className="block text-sm font-semibold uppercase tracking-wide text-slate-600">
                  ...
                  <input
                    ...
                    min={new Date(employmentRecord.dateJoin ?? "")
                      .toISOString()
                      .slice(0, 10)}
                    onChange={(event) => setLastWorkingDay(event.target.value)}
                    className="mt-2 h-10 w-full rounded-md border border-slate-200 px-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-red-500 focus:ring-2 focus:ring-red-100"
                    required
                  />
                </label>
              ) : (
                <div className="flex gap-3 rounded-lg border border-red-200 bg-red-50 p-4 text-red-700">
                  ...
                  <div>
                    <p className="font-semibold">No Employment Record Found</p>
                    <p className="mt-1">
                      This user does not have a Date Join in their employment
                      profile. Update employment details before marking them as
                      resigned.
                    </p>
                  </div>
                </div>
          ...
          {["Suspend User", "Delete User", "Activate User"].includes(
            action,
          ) && (
            <p>
          ...
          <footer className="flex justify-end gap-2 border-t border-slate-100 pt-4">
            ...
            {hasSubmit && (
              <button
                type="button"
                disabled={
                  saving || (action === "Resign User" && !employmentRecord)
                }
                onClick={() => void submit()}
...
function SiteAssignments({
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-xl border border-slate-200 py-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4 px-6">
          <div className="flex items-center gap-3">
            <span className="grid size-14 shrink-0 place-items-center rounded-full bg-blue-600 text-lg font-semibold text-white">
            ...
            <div>
              ...
              <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-slate-500">
                <span
                  className={`rounded-md border px-2 py-0.5 font-medium ${user.role === "OM" ? "border-blue-200 bg-blue-100 text-blue-700" : "border-emerald-200 bg-emerald-100 text-emerald-700"}`}
                >
                  {roleLabel}
                </span>
                <span>{user.id}</span>
          ...
          <span
            className={`rounded-md px-3 py-1 text-sm font-medium ${statusClass}`}
          >
            {user.status}
          </span>
        </div>
        <div className="mt-5 grid gap-3 px-6 text-sm text-slate-500 sm:grid-cols-2">
      ...
      <section>
        ...
        {loading ? (
          <div className="rounded-lg border px-4 py-10 text-center text-slate-500">
          ...
          <div className="overflow-x-auto rounded-lg border border-slate-200">
            <table className="min-w-full text-left">
              <thead className="border-b bg-slate-50 text-xs uppercase tracking-wider text-slate-500">
                <tr>
                  <th className="px-4 py-4 font-medium">Site Name</th>
                  <th className="px-4 py-4 font-medium">Address</th>
                  <th className="px-4 py-4 font-medium">Site Code</th>
              ...
              <tbody>
                {sites.map((site) => (
                  ...
                  >
                    ...
                    <td
                      className="max-w-sm truncate px-4 py-4 text-slate-500"
                      title={site.address}
                    >
                      {site.address || "—"}
                    </td>
                    <td className="px-4 py-4 text-slate-600">{site.code}</td>
          ...
          <div className="flex min-h-64 items-center justify-center rounded-lg border p-8 text-center md:max-h-[30vh]">
            <div>
              <MapPin className="mx-auto mb-3 size-12 text-slate-400" />
              <h4 className="mb-1 text-lg font-medium text-slate-900">
                No Sites Assigned
              </h4>
              <p className="text-slate-500">
                This user has not been assigned to any sites yet.
              </p>
            </div>
          </div>
        )}
      ...
      {sites.length ? (
        <div className="max-h-[40vh] space-y-3 overflow-y-auto pr-1">
          {sites.map((site) => (
            <label
              key={site.id}
              className="flex cursor-pointer items-center gap-4 rounded-xl border border-slate-200 px-4 py-4 transition hover:border-slate-300 hover:bg-slate-50"
            >
              <input
                type="checkbox"
                checked={selected.includes(site.id)}
                onChange={() => onToggle(site.id)}
                className="size-5 rounded border-slate-300 text-slate-900 focus:ring-slate-700"
              />
              <MapPin className="size-5 shrink-0 text-slate-500" />
              <span className="min-w-0">
                <b className="block text-base text-slate-900">{site.name}</b>
        ...
        <div className="rounded-xl border border-dashed border-slate-200 py-10 text-center text-slate-500">
          No active sites are available.
        </div>
      )}
      <p className="mt-5 text-sm text-slate-500">
        Selected: {selected.length} of {sites.length} sites
      </p>
    </div>
  );
}
    ...
    <dl className="space-y-3">
      {rows.map(([label, value]) => (
        <div key={label} className="grid grid-cols-3 gap-3">
          ...
          <dd className="col-span-2 wrap-break-word text-slate-900">
            {value || "—"}
          </dd>
        </div>
      ))}
