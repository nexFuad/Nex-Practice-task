...
const includeClients = { siteClients: { include: { client: true } } };
...
sitesRoutes.post("/", async (c) => {
    ...
    if (!input.name?.trim() || !input.code?.trim() || !input.address?.trim())
        return c.json({ message: "Name, code, and address are required." }, 400);
...
sitesRoutes.get("/:id/assigned-guards", async (c) => {
    ...
    if (!site)
        return c.json({ message: "Site not found." }, 404);
    return c.json(site.userSites.map(({ user, assignedAt }) => ({
        id: user.id,
...
sitesRoutes.put("/:id/clients", async (c) => {
    ...
    if (!Array.isArray(clientIds) || clientIds.some((id) => typeof id !== "string"))
        return c.json({ message: "clientIds must be an array of client IDs." }, 400);
    ...
    if (!site)
        return c.json({ message: "Site not found." }, 404);
    if (clients !== uniqueClientIds.length)
        return c.json({ message: "One or more clients do not exist." }, 400);
...
sitesRoutes.put("/:id", async (c) => {
    try {
        return c.json(await prisma.site.update({ where: { id: c.req.param("id") }, data: normalizeSite(await c.req.json()), include: includeClients }));
    }
    catch {
        return c.json({ message: "Site not found or code already exists." }, 404);
    }
});
...
sitesRoutes.delete("/:id", async (c) => {
    try {
        await prisma.site.delete({ where: { id: c.req.param("id") } });
