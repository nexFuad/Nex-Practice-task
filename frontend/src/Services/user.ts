...
export async function getUsers(
  ...
  const response = await request<
    Omit<PaginatedUsers, "items"> & { items: ApiUser[] }
  >(`${apiBaseUrl}/api/users?${params.toString()}`);
  return { ...response, items: response.items.map(asUser) };
}
export function getUserFilterOptions() {
  return request<UserFilterOptions>(`${apiBaseUrl}/api/users/filter-options`);
}
...
export function saveUserSites(employeeId: string, siteIds: string[]) {
  return request<AssignedSite[]>(
    `${apiBaseUrl}/api/users/${encodeURIComponent(employeeId)}/sites`,
    {
      method: "PATCH",
...
export function getUserSchedule(employeeId: string) {
  return request<UserScheduleRecord[]>(
    `${apiBaseUrl}/api/users/${encodeURIComponent(employeeId)}/schedule`,
  );
}

export function resetUserPassword(employeeId: string, password: string) {
  return request<{ message: string }>(
    `${apiBaseUrl}/api/users/${encodeURIComponent(employeeId)}/password`,
    {
      method: "PATCH",
...
export function setUserStatus(
  ...
  return request<{ message: string }>(
    `${apiBaseUrl}/api/users/${encodeURIComponent(employeeId)}/status`,
    {
      method: "PATCH",
...
export function updateUserPayroll(userId: string, payload: PayrollPayload) {
  return request<PayrollPayload>(
    `${apiBaseUrl}/api/users/${encodeURIComponent(userId)}/payroll`,
    { method: "PUT", body: JSON.stringify(payload) },
  );
