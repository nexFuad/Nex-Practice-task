...
export function ShiftFormModal({
  ...
  const [values, setValues] = useState<ShiftPayload>(() =>
    shift === "new"
      ? defaultValues
      : {
          companyId: shift.companyId ?? "default",
          name: shift.name,
          code: shift.code,
          category: shift.category ?? "Main",
  ...
  const set = <K extends keyof ShiftPayload>(
    key: K,
    value: ShiftPayload[K],
  ) => {
    setValues((current) => {
      const next = { ...current, [key]: value };
      if (!durationEdited && (key === "startTime" || key === "endTime"))
        next.durationHours = calculateDuration(
  ...
  const validate = () => {
    ...
    if (!/^#[0-9a-fA-F]{6}$/.test(values.color))
      next.color = "Use a valid hex value, such as #E5E7EB.";
    if (
      !Number.isFinite(Number(values.durationHours)) ||
      Number(values.durationHours) < 0
    )
      next.durationHours = "Duration must be zero or greater.";
    setErrors(next);
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setErrors({
        form:
          cause instanceof Error ? cause.message : "Unable to save shift type.",
      });
    } finally {
      setSaving(false);
    }
  ...
  const inputClass = (error?: string) =>
    `mt-1.5 h-10 w-full rounded-md border bg-white px-3 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100 ${error ? "border-red-400" : "border-slate-200"}`;
  const errorText = (error?: string) =>
    error ? <p className="mt-1 text-xs text-red-600">{error}</p> : null;
  return (
    <div
      className="fixed inset-0 z-70 grid place-items-center bg-slate-950/45 p-3 sm:p-6"
      role="dialog"
      aria-modal="true"
      aria-labelledby="shift-type-title"
    >
      <form
        onSubmit={submit}
        className="relative flex max-h-[95vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl"
      >
        <header className="shrink-0 border-b border-slate-100 px-5 py-5 sm:px-7">
          ...
          >
            {shift === "new" ? "Create Shift Type" : "Edit Shift Type"}
          </h2>
          <p className="mt-1 text-sm text-slate-500">
            Define the details for a shift template. Codes must be unique per
            company.
          </p>
        </header>
        <div className="flex-1 overflow-y-auto px-5 py-5 sm:px-7">
          ...
          <div className="grid gap-x-6 gap-y-4 sm:grid-cols-2">
            ...
            <label className="text-sm font-medium text-slate-800">
              ...
              <span className="mt-1.5 flex items-center gap-2">
                <input
                  type="color"
                  aria-label="Choose shift colour"
                  value={values.color}
                  onChange={(event) =>
                    set("color", event.target.value.toUpperCase())
                  }
                  className="size-10 shrink-0 cursor-pointer rounded-md border border-slate-200 bg-white p-1"
                />
                <input
                  value={values.color}
            ...
            <label className="text-sm font-medium text-slate-800">
              ...
              <span className="mt-1 block text-xs font-normal text-slate-500">
                Calculated from the selected times; you can adjust it manually.
              </span>
          ...
          <label className="mt-4 block text-sm font-medium text-slate-800">
            ...
            <textarea
              ...
              className="mt-1.5 min-h-24 w-full rounded-md border border-slate-200 px-3 py-2 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
              placeholder="Optional description"
            />
          </label>
        </div>
        <footer className="flex shrink-0 justify-end gap-3 border-t border-slate-100 px-5 py-4 sm:px-7">
          <button
            type="button"
            onClick={onClose}
            className="h-10 rounded-md border border-slate-200 px-5 text-sm font-medium hover:bg-slate-50"
          >
            Cancel
          </button>
          <button
            disabled={saving}
            className="h-10 rounded-md bg-neutral-950 px-5 text-sm font-medium text-white hover:bg-neutral-800 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {saving
              ? "Saving..."
              : shift === "new"
