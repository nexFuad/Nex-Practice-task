...
export function SiteManagement() {
  ...
  return <section onClick={() => setOpenMenuId(null)} className="min-w-0 px-5 pb-5 pt-2 text-slate-800">{toast && <Toast message={toast} onClose={() => setToast(null)} />}<div className="space-y-6"><header className="flex items-center justify-between gap-4"><div><h1 className="text-2xl font-semibold text-slate-800">Site Management</h1><p className="text-sm text-slate-500">Manage security sites and locations</p></div><button type="button" onClick={() => setFormSite("new")} className="inline-flex h-9 items-center gap-2 rounded-md bg-neutral-900 px-4 text-sm font-medium text-white shadow-sm transition hover:bg-neutral-800"><Plus className="size-4" />Add Site</button></header><SiteStats values={metrics} /><section className="flex flex-col gap-4 rounded-xl border border-slate-200 bg-white py-6 shadow-sm"><header className="px-6"><h2 className="font-semibold">All Sites</h2><p className="mt-1 text-sm text-slate-500">Manage site locations and assignments</p></header><div className="space-y-4 px-6">{error && <p role="alert" className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>}<div className="flex flex-wrap gap-4"><label className="relative min-w-56 flex-1"><Search className="absolute left-2 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Search sites..." className="h-9 w-full rounded-md border border-slate-200 bg-white py-1 pl-8 pr-3 text-sm shadow-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100" /></label><select value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); }} className="h-9 w-45 rounded-md border border-slate-200 bg-white px-3 text-sm shadow-sm outline-none focus:border-blue-400"><option value="ALL">All Status</option><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></select><button type="button" onClick={exportCsv} className="inline-flex h-9 items-center gap-2 rounded-md border border-slate-200 bg-white px-4 text-sm font-medium shadow-sm hover:bg-slate-50"><Download className="size-4" />Export</button></div>{isLoading ? <p className="py-10 text-center text-sm text-slate-500">Loading sites...</p> : <><SitesTable sites={paginatedSites} openMenuId={openMenuId} onToggleMenu={(id) => setOpenMenuId((current) => current === id ? null : id)} onView={(site) => { setDetailsSite(site); setOpenMenuId(null); }} onEdit={(site) => { setFormSite(site); setOpenMenuId(null); }} onManageClients={(site) => { setClientSite(site); setOpenMenuId(null); }} onToggleStatus={toggleStatus} onDelete={removeSite} /><UsersPagination page={currentPage} totalPages={totalPages} onPageChange={setPage} /></>}</div></section></div>{formSite && <SiteFormModal site={formSite} onClose={() => setFormSite(null)} onSubmit={saveSite} />}{detailsSite && <SiteDetailsModal site={detailsSite} onClose={() => setDetailsSite(null)} />}{clientSite && <ManageSiteClientsModal site={clientSite} onClose={() => setClientSite(null)} onSaved={saveClients} />}</section>;
}
